...
static double computeInertia(observation pts[], size_t len, cluster cent[], int k)
{
    (void)k;
    double inertia = 0.0;
    ...
    {
        int g = pts[j].group;
        double dx = pts[j].x - cent[g].x;
        double dy = pts[j].y - cent[g].y;
        inertia += dx * dx + dy * dy;
    }
 ...
 * Generates 2000000 points in a circle of
 * radius 20.0 with center at (0,0)
 * and cluster them into 16 clusters
 *
...
static void test()
{
    ...
    int k = 16;  // No of clusters
    cluster* clusters = kMeans(observations, size, k);
    double inertia = computeInertia(observations, size, clusters, k);
    printf("inertia = %.6f\n", inertia);
